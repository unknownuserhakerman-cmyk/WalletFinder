// ================================================================
// FULL BIP39 ENGINE — Pure JS, works on any device
// ================================================================
...
"canyon","capable","capital","captain","car","carbon","carpet","carry","cart","case",
...
"defense","define","defy","degree","delay","deliver","demand","denial","deny","depart",
...
// SHA-256 pure JS
function sha256(str) {
    function rrot(v, s) { return (v >>> s) | (v << (32 - s)); }
    ...
    let bytes = [];
    for (let i = 0; i < str.length; i++) { let c = str.charCodeAt(i); if (c<128) bytes.push(c); else if (c<2048) { bytes.push(192|(c>>6)); bytes.push(128|(c&63)); } else { bytes.push(224|(c>>12)); bytes.push(128|((c>>6)&63)); bytes.push(128|(c&63)); } }
    let len = bytes.length * 8;
    bytes.push(0x80); while ((bytes.length % 64) !== 56) bytes.push(0);
    for (let i=7;i>=0;i--) bytes.push((len>>>(i*8))&255);
    ...
    for (let i=0;i<bytes.length;i+=64) {
        ...
        for (let t=16;t<64;t++) { let s0=rrot(W[t-15],7)^rrot(W[t-15],18)^(W[t-15]>>>3); let s1=rrot(W[t-2],17)^rrot(W[t-2],19)^(W[t-2]>>>10); W[t]=(W[t-16]+s0+W[t-7]+s1)&0xffffffff; }
        let a=H[0],b=H[1],c=H[2],d=H[3],e=H[4],f=H[5],g=H[6],h=H[7];
        for (let t=0;t<64;t++) { let S1=rrot(e,6)^rrot(e,11)^rrot(e,25); let ch=(e&f)^((~e)&g); let t1=(h+S1+ch+K[t]+W[t])&0xffffffff; let S0=rrot(a,2)^rrot(a,13)^rrot(a,22); let maj=(a&b)^(a&c)^(b&c); let t2=(S0+maj)&0xffffffff; h=g;g=f;f=e;e=(d+t1)&0xffffffff;d=c;c=b;b=a;a=(t1+t2)&0xffffffff; }
    ...
    return Array.from(H).map(v=>('00000000'+v.toSt
